 ...
 */
public class AdminCaracter extends javax.swing.JFrame {

    Caracter tp = null;
    EntityManagerFactory emf = null;
    CaracterJpaController jpa;
    
    public AdminCaracter() {
        initComponents();
        this.emf = Persistence.createEntityManagerFactory("SyncPetPU");
        this.setLocationRelativeTo(null);
        this.jpa = new CaracterJpaController(emf);
        rellenarTabla();
        deshabilitarCampos();
    }
    ...
    public void rellenarTabla() {
        ...
        for(Caracter te : jpa.findCaracterEntities()) {
            Object[] obj = new Object[2];
            obj[0] = te.getIdCaracter();
            obj[1] = te.getNombre();
            modelo.addRow(obj);
        }
    ...
    public void rellenarCampos(String nombre) {
        vaciarCampos();
    ...
    private void initComponents() {
        ...
        setTitle("SyncPet :: Administrar Carácter");
        setResizable(false);
        ...
        lblNombre.setText("Nombre de Carácter");
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Tipo de Carácter"));
    ...
    private void btnRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoverActionPerformed
        ...
        if(jpa.findCaracter(Integer.parseInt(String.valueOf(modelo.getValueAt(tablaResultados.getSelectedRow(), 0)))) != null) {
            int opcion = preguntar("¿Esta seguro de eliminar el tipo de carácter?");
            if(opcion == 0) {
                try {
                    ...
                    mostrarError("No se pudo encontrar el tipo de carácter en la base de datos porque ya no existe");
                    btnCancelarActionPerformed(evt);
                }
            }
        }else{
    ...
    private void tablaResultadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaResultadosMouseClicked
        DefaultTableModel modelo = (DefaultTableModel)tablaResultados.getModel();
        //consultando por el valor a cargar, es recuperado desde el valor seleccionado recuperando el ROW ID
        tp = jpa.findCaracter(Integer.parseInt(String.valueOf(modelo.getValueAt(tablaResultados.getSelectedRow(), 0))));
        if(tp == null) {
            this.mostrarError("El tipo de fármaco no pudo ser hallado por el sistema");
        }else{
            habilitarCampos();
            rellenarCampos(tp.getNombre());
        ...
        if(btnAccion.getText().compareToIgnoreCase("guardar") == 0) {
            ...
            if(!esVacio(txtNombre)) {
                if(jpa.existeTipo(txtNombre.getText())) {
                    this.mostrarError("El tipo de carácter "+contenido(txtNombre)+" ya se encuentra registrado en la base de datos");
                    txtNombre.selectAll();
                    txtNombre.requestFocus();
                }else{
                    try {
                        jpa.create(new Caracter(jpa.ultimo(), contenido(txtNombre)));
                        mostrarMensaje("Carácter guardado");
                        btnCancelarActionPerformed(evt);
                    }catch(Exception e) {
                        mostrarError("Ocurrió un error al intentar registrar el tipo de carácter en el sistema: "+e.getMessage());
                    }
                }
            }
        }else{
                ...
                if(jpa.existeTipo(txtNombre.getText())) {
                    try {
                        if(jpa.buscarPorNombre(contenido(txtNombre)) == null) {
                            ...
                            if(jpa.buscarPorNombre(contenido(txtNombre)).getIdCaracter()== tp.getIdCaracter()) {
                                tp.setNombre(contenido(txtNombre));
                                jpa.edit(tp);
                                mostrarMensaje("Actualizado");
                                tp = null;
                                ...
                                mostrarError("El tipo de carácter ya se encuentra registrado en la base de datos");
                                txtNombre.selectAll();
                                txtNombre.requestFocus();
                            }
                        }
                    }catch(Exception e) {
                        mostrarError("Ha ocurrido un error al intentar actualizar el tipo de carácter: "+e.getMessage());
                    }
                }else{
                    try {
                        tp.setNombre(contenido(txtNombre));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AdminCaracter.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
