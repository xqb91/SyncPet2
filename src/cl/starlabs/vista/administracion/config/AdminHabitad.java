...
public class AdminHabitad extends javax.swing.JFrame {
    ...
    public void rellenarTabla() {
        DefaultTableModel modelo = new DefaultTableModel(new Object [][] { }, new String [] { "ID", "Carácter" });
        ...
        for(Habitad te : jpa.findHabitadEntities()) {
            Object[] obj = new Object[2];
            obj[0] = te.getIdHabitad();
            obj[1] = te.getNombre();
            modelo.addRow(obj);
        }
    ...
    public void rellenarCampos(String nombre, String descripcion) {
        vaciarCampos();
    ...
    private void initComponents() {
        ...
        setTitle("SyncPet :: Administrar Habitad");
        setResizable(false);
    ...
    private void btnRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoverActionPerformed
        ...
        if(jpa.findHabitad(Integer.parseInt(String.valueOf(modelo.getValueAt(tablaResultados.getSelectedRow(), 0)))) != null) {
            int opcion = preguntar("¿Esta seguro de eliminar el tipo de habitad?");
            if(opcion == 0) {
                try {
                    ...
                    mostrarError("No se pudo encontrar el tipo de habitad en la base de datos porque ya no existe");
                    btnCancelarActionPerformed(evt);
                }
            }
        }else{
    ...
    private void tablaResultadosKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tablaResultadosKeyTyped

    }//GEN-LAST:event_tablaResultadosKeyTyped
            ...
            if(!esVacio(txtNombre)) {
                if(!esVacio(txtDescripcion)) {
                    if(jpa.existeTipo(txtNombre.getText())) {
                        this.mostrarError("El tipo de habitad "+contenido(txtNombre)+" ya se encuentra registrado en la base de datos");
                        txtNombre.selectAll();
                        txtNombre.requestFocus();
                    }else{
                        try {
                            jpa.create(new Habitad(jpa.ultimo(), contenido(txtNombre)));
                            mostrarMensaje("Hábitad guardado");
                            btnCancelarActionPerformed(evt);
                        }catch(Exception e) {
                            mostrarError("Ocurrió un error al intentar registrar el tipo de habitad en el sistema: "+e.getMessage());
                        }
                    }
                }
            }
                    ...
                    if(jpa.existeTipo(txtNombre.getText())) {
                        try {
                            if(jpa.buscarPorNombre(contenido(txtNombre)) == null) {
                                ...
                                if(jpa.buscarPorNombre(contenido(txtNombre)).getIdHabitad()== tp.getIdHabitad()) {
                                    tp.setNombre(contenido(txtNombre));
                                    tp.setDescripcion(contenido(txtDescripcion));
                                    jpa.edit(tp);
                                    mostrarMensaje("Actualizado");
                                    ...
                                    mostrarError("El tipo de habitad ya se encuentra registrado en la base de datos");
                                    txtNombre.selectAll();
                                    txtNombre.requestFocus();
                                }
                            }
                        }catch(Exception e) {
                            mostrarError("Ha ocurrido un error al intentar actualizar el tipo de habitad: "+e.getMessage());
                        }
                    }else{
                        try {
                            tp.setNombre(contenido(txtNombre));
    ...
    private void txtNombreKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtNombreKeyTyped
        largoMaximo(txtNombre, 75, evt);
    }//GEN-LAST:event_txtNombreKeyTyped

    private void tablaResultadosMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaResultadosMouseEntered

    }//GEN-LAST:event_tablaResultadosMouseEntered

    private void tablaResultadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaResultadosMouseClicked
        DefaultTableModel modelo = (DefaultTableModel)tablaResultados.getModel();
        //consultando por el valor a cargar, es recuperado desde el valor seleccionado recuperando el ROW ID
        tp = jpa.findHabitad(Integer.parseInt(String.valueOf(modelo.getValueAt(tablaResultados.getSelectedRow(), 0))));
        if(tp == null) {
            this.mostrarError("El tipo de fármaco no pudo ser hallado por el sistema");
        }else{
            habilitarCampos();
            rellenarCampos(tp.getNombre(), tp.getDescripcion());
